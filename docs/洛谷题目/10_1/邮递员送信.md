...
// 开两个邻接表：adj1 存原图，adj2 存反向图
vector<Edge> adj1[MAXN];
vector<Edge> adj2[MAXN];

// 传入指定的邻接表和距离数组进行 Dijkstra
void dijkstra(int s, int n, const vector<Edge> adj[], int dist[]) {
    for (int i = 1; i <= n; i++) {
        dist[i] = INF;
    }
    dist[s] = 0;
...
int dist1[MAXN]; // 记录 1 到 i 的最短路 (去程)
int dist2[MAXN]; // 记录 i 到 1 的最短路 (回程)
...
int main() {
    ...
    // 1. 在原图上跑 Dijkstra，得到所有去程时间
    dijkstra(1, n, adj1, dist1);
    
    // 2. 在反向图上跑 Dijkstra，得到所有回程时间
    dijkstra(1, n, adj2, dist2);

    // 3. 统计总时间 (注意：总时间可能会超出 int 范围，必须用 long long)
    long long total_time = 0;
    for (int i = 2; i <= n; i++) {
        total_time += dist1[i] + dist2[i];
    }
